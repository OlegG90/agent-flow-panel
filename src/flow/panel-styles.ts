...
.step { padding: 0.4rem 0.6rem; border-radius: 6px; border: 1px solid transparent; margin-top: 0.3rem; cursor: pointer; }
...
/* Newest-first is a display flip only: units keep their order in the DOM, so
   ...
   The flip now covers both the units and the steps inside each unit, so the
   newest event in a block (e.g. Answer) lands on top, before Model calls. */
.flow.newest-first { flex-direction: column-reverse; }
.flow.newest-first .steps { flex-direction: column-reverse; }
.flow.newest-first .steps--nested { flex-direction: column-reverse; }
.details {
  flex: 2;
  min-width: 0;
  position: sticky;
