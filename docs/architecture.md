...
Note the observation gap: omp's `-p` mode exits as soon as the answer lands, taking the panel with it, and a slash command passed to `-p` reaches the model as ordinary text rather than being dispatched — `omp -p "/flow"` handed `/flow` to the model, which went looking for a `git flow` binary. A **live** omp panel therefore has to be opened from inside the interactive TUI.

omp itself, though, is scriptable through `--mode=rpc`: newline-delimited JSON on stdin, where `type` names the rpc command and the text goes in `message` (`text`/`prompt`/`input` all fail with `e.trimStart`). It answers with an `available_commands_update` frame carrying the whole command registry and, for each entry, a `source` of `builtin` / `skill` / `extension` / `custom` / `file` — which is how the `.claude/commands/` shadowing was established rather than guessed. `prompt_result{agentInvoked}` says whether the model was involved at all, and `--config <overlay.yml>` tries a configuration without touching the real one.
